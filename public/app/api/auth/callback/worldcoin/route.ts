...
export async function GET(req: NextRequest){
  ...
  if (!tokenRes.ok) return NextResponse.redirect(new URL('/?error=token_exchange_failed', url.origin))
  const tokenJson = await tokenRes.json() as any
  let profile: any = {}
  if (tokenJson.access_token){
    const infoRes = await fetch('https://id.worldcoin.org/userinfo', { headers: { Authorization: `Bearer ${tokenJson.access_token}` } })
    if (infoRes.ok) profile = await infoRes.json()
  }
  setSessionCookie({ sub: profile.sub || 'unknown', name: profile.name, email: profile.email })
