...
// 👇 Flag para mostrar/ocultar el demo
...
const RAINBOW = {
  ...
  DECOY_CHANCE: 0.25,          // probabilidad de distractor durante el desafío
...
if (now < combo.frenzyUntil){
  ...
  // 👉 NUEVO: durante Frenzy sí suena tick.mp3
...
let comboTxtEl = document.getElementById('comboTxt');
...
playSnd('tension', { volume: 0.7, loop: true });
  ...
  challenge.active = true;
    updateBadge();  // 👈 muestra “RAINBOW RACE” con pulso al iniciar
...
async function handleVote(key){
  ...
  try {
    ...
    console.error("Error al enviar voto al backend:", e);
  }

  // === Mantener tu lógica local ===
...
if (sendSuggestBtn){
  sendSuggestBtn.onclick = async () => {
    ...
    try {
      ...
      console.error("Error al enviar sugerencia al backend:", e);
    }

    // === Mantener tu UI local ===
...
if (payIdeasBtn){
  payIdeasBtn.onclick = async (ev) => {
    ...
    if (window.WG_DEV_MODE){
      {
        const cur = getLang(); const T = I18N[cur] || I18N.es;
          alert(T.ticket_bought_dev || "✅ Ticket comprado (simulación DEV)");
       }
   ...
   try{
     ...
     if (res.status !== "success"){
      ...
      alert("✅ Ticket Ideas comprado con éxito");
      ideasPayView.style.display = 'none';
      ideasOptionsView.style.display = 'block';
      ideasTicketActive = true;
      startTicketTimer(5);
      ...
      alert("⚠️ Hubo un problema con el pago");
    }
  };
}
...
function addTap(){ if(energy<1) return false; energy-=1; rbgp+=POWER_BASE; localStorage.setItem('energy',String(energy)); localStorage.setItem('rbgp',String(rbgp)); return true; }
// Anti-autotap: rate limit
let lastTapTime = 0;
let tapCount = 0;

coin.addEventListener('click', (e) => {
  if (!window.VERIFIED) return; // 🔒 sin verificación no hay tap
    // 🔒 Anti-autotap: ignorar clicks sintéticos
  if (!e.isTrusted) return;
    const nowMs = Date.now();
  if (nowMs - lastTapTime < 1000) {
    tapCount++;
    if (tapCount > 12) return; // más de 12 taps en un segundo → ignorar
  } else {
    tapCount = 1;
    lastTapTime = nowMs;
  }
  ...
  spawnGain?.(cx, cy, `+${fmt(POWER_BASE)}`);
  applyComboTap(e);
 // 👇 Cola servidor: sumamos 1 tap (se sincroniza en lote)
  tapQueue++;
  maybeSpawnByTap?.();
  render?.();
  updateRefillCue();
...
// =====================================
// Username (guardar + límite 3 cambios)
// =====================================

// variable global inicial
var username = localStorage.getItem("username") || "Player";

// getUsername siempre devuelve la variable global
...
function canChangeName(){
  const n = +localStorage.getItem("username_changes") || 0;
  return n < 3;
}

// guardar nombre con límite
async function setUsernameLimited(v){
  if (!canChangeName()){
    alert("❌ Ya alcanzaste el máximo de 3 cambios de nombre.");
    return getUsername();
  }

  const s = (v || "").slice(0,20);

  // simulamos guardar en backend (o fallback local)
  username = s;
  localStorage.setItem("username", s);

  const n = +localStorage.getItem("username_changes") || 0;
  localStorage.setItem("username_changes", String(n+1));
    ...
    if (saveBtn){
      saveBtn.onclick = async ()=>{
        ...
        if (savedBadge){
          ...
          savedBadge._t = setTimeout(()=> savedBadge.style.display='none', 2000);
        }
      };
    }
  }
...
const I18N = {
  ...
  // --- Ideas / Boosters / Varios ---
    ...
    sug_sent_private: '✅ Sugerencia enviada (privada)',
...
function loadProfileFields(){
 ...
 if (usernameInput && !usernameInput.value){
  ...
  saveBtn.onclick = async ()=>{
    const normalized = await setUsernameLimited(usernameInput.value);
    username = normalized; // 👈 forzamos que la variable global se actualice
    usernameInput.value = normalized; // 👈 forzamos que el input se quede con lo nuevo
    saveBtn.style.display = 'none';
    if (savedBadge){
      savedBadge.style.display = 'inline';
      clearTimeout(savedBadge._t);
...
function renderProfile(){
  ...
  // Si mañana agregas un span para mostrar el username en el perfil, lo actualizas aquí.
...
updateBadge(); // inicia oculto en base
  ...
  (function(){
    ...
    imgs.forEach(img => {
      // Evita tocar las del splash crítico
      if (img.closest('#splash')) return;
      try {
        img.loading  = 'lazy';
        img.decoding = 'async';
        // para móviles antiguos sin soporte 'loading', no pasa nada
      } catch(_) {}
    });
  })();
